...
var virtualRow = function (name) {
    ...
    var ret = new Proxy(ident, {
        construct(target, args) {
            return SQLFunction.fromArgs(args);
        },
        apply(target, thisArg, argumentsList) {
            if (argumentsList.Length) {
                return SQLFunction.fromArgs([property].concat(args));
            } else {
                target;
            }
        },
        get(object, property) {
            if (Reflect.has(object, property)) {
                ...
                return (args) => { return SQLFunction.fromArgs([m, name].concat(args)); }
            }
        }
    });
...
var addDateMethod = function (op) {
    return (args) => this.date[op].apply(this.date, ...args);
};
...
Object.defineProperty(sql, "patio", () => {
